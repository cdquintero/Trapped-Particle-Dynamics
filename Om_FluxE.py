...
def POPULATE_ARRAYS(n,m,j,k) :
    ...
    Map_prtnsmax= 0
    Prtnsmax_name = os.path.normpath(r'C:\Users\cd67\Desktop\David\1 Documentos\Proyecto de Grado\Program\ap8max.txt')
    ...
    Map_prtnsmin= 0
    Prtnsmin_name = os.path.normpath(r'C:\Users\cd67\Desktop\David\1 Documentos\Proyecto de Grado\Program\ap8min.txt')
    ...
    Map_eltnsmin = 0
    Eltnsmin_name = os.path.normpath(r'C:\Users\cd67\Desktop\David\1 Documentos\Proyecto de Grado\Program\ae8min.txt')
    ...
    Map_eltnsmax = 0
    Eltnsmax_name = os.path.normpath(r'C:\Users\cd67\Desktop\David\1 Documentos\Proyecto de Grado\Program\ae8max.txt')
...
Num_BB0 = 5 
Range=10000                                                              #Número de energías
BB0 = np.empty(Num_BB0)
Flux= np.empty(Range)
Omp_Fluxmin = np.empty([Range, Num_BB0]) 
Omp_Fluxmax = np.empty([Range, Num_BB0])     
Ome_Fluxmin = np.empty([Range, Num_BB0]) 
Ome_Fluxmax = np.empty([Range, Num_BB0])                                                                                                       
mProt= np.empty(Range)                                                   
mElct= np.empty(Range)                                                  
...
L_value = 2.5                                           #Cálculo del valor de McIlwain
En = np.linspace(En_start, En_end, num=Range)                                                 #Campo magnético normalizado al valor ecuatorial
Ene = np.linspace(Ene_start, Ene_end, num=Range)                                                 #Campo magnético normalizado al valor ecuatorial
...
ii=np.arange(Range)
iB=np.arange(Num_BB0)


for jj in zip(BB0, iB) :                                                 
    B = jj[0]                                       #Campo magnético normalizado al valor ecuatorial
    TRARA1(L_value, B, En, Flux, Range, Descr_prtnsmin, Mapmin)
    for kk in zip(Flux, ii) :
        mProt[kk[1]]=(10.0**(kk[0]))                                                    #Flujo Omndirecconal
    Omp_Fluxmin[:,jj[1]] = mProt 
    
for jj in zip(BB0, iB) :                                                 
    B = jj[0]                                        #Campo magnético normalizado al valor ecuatorial
    TRARA1(L_value, B, En, Flux, Range, Descr_prtnsmax, Mapmax)
    for kk in zip(Flux, ii) :
        mProt[kk[1]]=(10.0**(kk[0]))                                                    #Flujo Omndirecconal
...
for jj in zip(BB0, iB) :                                                 
    B = jj[0]                                       #Campo magnético normalizado al valor ecuatorial
    TRARA1(L_value, B, Ene, Flux, Range, Descr_eltnsmin, Mapmin)
    for kk in zip(Flux, ii) :
...
for jj in zip(BB0, iB) :                                                 
    B = jj[0]                                        #Campo magnético normalizado al valor ecuatorial
    TRARA1(L_value, B, Ene, Flux, Range, Descr_eltnsmax, Mapmax)
    for kk in zip(Flux, ii) :
        mElct[kk[1]]=(10.0**(kk[0]))                                                    #Flujo Omndirecconal
...
for i in iB :    
    ax.plot(En, Omp_Fluxmin[:,i], label= 'min %10.2f' % (BB0[i]))   
    ax.plot(En, Omp_Fluxmax[:,i], linestyle='--', color='black', label= 'max %10.2f' % (BB0[i]))

plt.rc('font', family='serif')
plt.ylabel(r'Flujo omnidireccional $[\frac{Protones}{cm^2 s}]$', size='large',  family='monospace', weight= 'light')
plt.xlabel(r'Energia de protones $[MEV]$',  size='large',  family='monospace', weight= 'light')
plt.legend()
plt.title(r'Flujo omnidireccional Vs E', size='large',  family='monospace', weight= 'light')
plt.legend()
ax.semilogx()
plt.xlim(0.1,10000) 
ax.semilogy()
...
for i in iB :    
    ax2.plot(Ene, Ome_Fluxmin[:,i], label= 'min %10.2f' % (BB0[i]))
    ax2.plot(Ene, Ome_Fluxmax[:,i], linestyle='--', label= 'max %10.2f' % (BB0[i]))

plt.rc('font', family='serif')
plt.ylabel(r'Flujo omnidireccional $[\frac{Electrones}{cm^2 s}]$', size='large',  family='monospace', weight= 'light')
plt.xlabel(r'Energia de electrones $[MEV]$',  size='large',  family='monospace', weight= 'light')
plt.legend()
plt.title(r'Flujo omnidireccional Vs E', size='large',  family='monospace', weight= 'light')
plt.legend()
ax2.semilogx()
...
fig.savefig(r'FVsBpL3.png', dpi=720) 
fig2.savefig(r'FVsBeL3.png.png', dpi=720) 
