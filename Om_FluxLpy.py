...
L_value = np.linspace(L_start, L_end, num=Range)                                         #Cálculo del valor de McIlwain
BB0 = 1                                                #Campo magnético normalizado al valor ecuatorial
...
for jj in zip(L_value, ii) :                                                 
    B = BB0                                       #Campo magnético normalizado al valor ecuatorial
    TRARA1(jj[0], B, Enp, Flux, Num_energ, Descr_prtnsmin, Mapmin)
...
for jj in zip(L_value, ii) :                                                 
    B = BB0                                       #Campo magnético normalizado al valor ecuatorial
    TRARA1(jj[0], B, Enp, Flux, Num_energ, Descr_prtnsmax, Mapmax)
    for kk in zip(Flux, iE) :
        mProt[kk[1]]=(10.0**(kk[0]))                                                    #Flujo Omndirecconal
...
for jj in zip(L_value, ii) :                                                 
    B = BB0                                       #Campo magnético normalizado al valor ecuatorial
    TRARA1(jj[0], B, Ene, Fluxe, Num_energe, Descr_eltnsmin, Mapmin)
...
for jj in zip(L_value, ii) :                                                 
    B = BB0                                       #Campo magnético normalizado al valor ecuatorial
    TRARA1(jj[0], B, Ene, Fluxe, Num_energe, Descr_eltnsmax, Mapmax)
    for kk in zip(Fluxe, iEe) :
        mElct[kk[1]]=(10.0**(kk[0]))                                                    #Flujo Omndirecconal
...
for i in iE :    
    ax.plot(L_value, Omp_Fluxmin[:,i], label= 'min %10.2f MeV' % (Enp[i]))   
    ax.plot(L_value, Omp_Fluxmax[:,i], linestyle='--', color='black', label= 'max%10.2f MeV' % (Enp[i]))

plt.rc('font', family='serif')
plt.ylabel(r'Flujo omnidireccional $[\frac{Protones}{cm^2 s}]$', size='large',  family='monospace', weight= 'light')
plt.xlabel(r'L',  size='large',  family='monospace', weight= 'light')
plt.legend()
...
for i in iEe :    
    ax2.plot(L_value, Ome_Fluxmin[:,i], label= 'min %10.2f MeV' % (Ene[i]))
    ax2.plot(L_value, Ome_Fluxmax[:,i], linestyle='--', color='black', label= 'max %10.2f MeV' % (Ene[i]))
...
fig.savefig(r'grafica1.15.png', dpi=720) 
